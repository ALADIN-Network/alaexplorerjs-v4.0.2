...
function writeApiGen(Network, network, structs, config) {
  ...
  for(let type in Network.schema) {
    ...
    if(type === 'transaction') {
      continue // https://github.com/ALADIN-Network/ala/issues/730
    }
  ...
  /**
    ...
    @example ala.contract('mycontract', [options], [callback])
    @example ala.contract('mycontract').then(mycontract => mycontract.action(...))
  */
  merge.contract = (...args) => {
...
function WriteApi(Network, network, config, Transaction) {
  ...
  */
  ...
  function genMethod(type, definition, struct, transactionArg, code = 'ala') {
  ...
  */
  function trMessageCollector(trCallback, options = {}, merges) {
    ...
    // merges can be an object of functions (as in the main ala contract)
    // or an object of contract names with functions under those
    for(const key in merges) {
      const value = merges[key]
      if(typeof value === 'function') {
        // Native operations (ala contract for example)
